...
- **Stage 7** `make_engine` returns `tuple[StreamingEngine | None, str | None]` instead of the design's bare `StreamingEngine | None`: the fallback reason must be programmatically available (not just logged) for Stage 8 to surface the one-per-run permission-denied pointer (R7) and for the stage's own matrix tests to assert reasons. A defensive `unknown_engine` reason was added for a corrupt `streaming_engine` config value.
- **Stage 9** the caption bar carries one slow (0.9s) cursor-blink `NSTimer`, active only while partial text is displayed, despite the stage step saying "no timer": R4/design §5 require a *blinking* cursor, and the design's actual constraint is "no 60fps timer of its own" — text redraws still happen only on text change.
- **Stage 11** the documented build command fails while `pyproject.toml` is hidden because `setup.py` reads the version from it; `APP_VERSION` must be set (CI already does): `APP_VERSION=$(grep -m1 '^version' pyproject.toml | sed 's/.*"\(.*\)"/\1/')` before hiding, then `mv pyproject.toml pyproject.toml.bak; APP_VERSION=$APP_VERSION python setup.py py2app; mv pyproject.toml.bak pyproject.toml`. Local builds also need `uv pip install py2app` plus the CI zlib patch (build.yml lines 140–147); `uv sync` restores the venv afterwards.
